...
class DashboardController {
    constructor() {
        this.summary = (0, wrap_1.warp)(async (req, res) => {
        ...
        this.monthlyCharts = (0, wrap_1.warp)(async (req, res) => {
            const userId = req.user._id;
            const year = Number(req.query.year) || new Date().getFullYear();
            const start = new Date(year, 0, 1);
            const end = new Date(year, 11, 31, 23, 59, 59, 999);
            // Pie chart: expenses per category
            const pieByCategory = await Transaction_1.Transaction.aggregate([
                {
                    $match: {
                        userId: userId,
                        type: "expense",
            ...
            const barByMonth = await Transaction_1.Transaction.aggregate([
                {
                    $match: {
                        userId: userId,
                        date: { $gte: start, $lte: end },
